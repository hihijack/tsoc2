...
class LineMaker extends ScriptableWizard {
	...
	static function CreateWizard () {
		...
		if (objectMesh.vertexCount > 1500) {
	...
	function ComputeLine (doVector3 : boolean) {
		...
		var output = "";
		for (i = 0; i < linePoints.Count; i++) {
			var p1 = Vector3Round(objectTransform.InverseTransformPoint(linePoints[i].p1));
			var p2 = Vector3Round(objectTransform.InverseTransformPoint(linePoints[i].p2));
			if (doVector3) {

...
function Start () {
	...
	Vector.DrawLine(controlLine);
	
	// Make the line object for the curve
	linePoints = new Vector2[segments+1];
	SetLine();
	// Create a curve in the VectorLine object
	Vector.MakeCurveInLine(line, curvePoints[0], curvePoints[1], curvePoints[2], curvePoints[3], segments);
...
function DrawLine () {
	...
	if (useDottedLine) {
		Vector.SetTextureScale(line, 1.0);
	}
}
...
function UpdateLine (objectNumber : int, pos : Vector2, go : GameObject) {
	...
	Vector.MakeCurveInLine(line, curvePoints[curveIndex], curvePoints[curveIndex+1], curvePoints[curveIndex+2], curvePoints[curveIndex+3], segments,
	...
	if (objectNumber % 2 == 0) {
		...
		// If it's not an end anchor point, move the next anchor/control points as well, and update the next curve
	 	if (objectNumber > 0 && objectNumber < curvePoints.Length-2) {
			...
			Vector.MakeCurveInLine(line, curvePoints[curveIndex+4], curvePoints[curveIndex+5], curvePoints[curveIndex+6], curvePoints[curveIndex+7], segments,
...
function AddPoint () {
	...
	if (line.lineVertices.Length + segments*4 > 65534) return;
	
	Resize.Array(linePoints, (segments+1) * ++numberOfCurves);
	Resize.Array(curvePoints, numberOfCurves*4);
	...
	Vector.MakeCurveInLine(line, curvePoints[pointIndex-4], curvePoints[pointIndex-3], curvePoints[pointIndex-2], curvePoints[pointIndex-1], segments,
...
function ChangeSegments () {
	...
	for (var i = 0; i < numberOfCurves; i++) {
		Vector.MakeCurveInLine(line, curvePoints[i*4], curvePoints[i*4+1], curvePoints[i*4+2], curvePoints[i*4+3], segments, (segments+1)*i);
	}
	DrawLine();
}
